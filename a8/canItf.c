...
static void CanSndMsg(CAN_ITF_STRU *pItf,char *pMsg, int nMsgLen)
{
    MAIN_CANITF_STRU *pCanCmd = (MAIN_CANITF_STRU *)pMsg;
...
static void CanItfTimerMsg(char *pMsg, int nMsgLen)
{
...
static void CanItfSecondhandler(void *arg)
{
    ...
    if (tm)
    {
        ...
        ret = SndMsg(MSG_QUEUE_ID_CANITF,(SAT_MSG_HEAD *)tm);
        if (0 != ret)
        {
            SatFreeMsg(tm);
        }
...
static int CanItfInit(void)
{
    ...
    dispatch_threadpool(gThdPool,CanItfRcvProc,&gCanItf[0]);
...
static void CanItfMsgProc(CAN_ITF_STRU *pItf,char *pMsg, int nMsgLen, int nEvent)
{
	switch(nEvent)
	{
	...
	case MAIN_CANITF_MSG:
        CanSndMsg(pItf,pMsg,nMsgLen);
        break;
    case TIMER_MSG:
        CanItfTimerMsg(pMsg,nMsgLen);
        break;
    default:
        break;
   }
...
static void CanRcvFrame(int iChl,unsigned char ucRcvBufIndex)
{
    ...
    MAIN_CANITF_STRU *pCanItfMsg = NULL;
	...
	pCanItfMsg = (MAIN_CANITF_STRU *)SatAllocMsg(MAIN_CANITF_MSG_SIZE + CanRcvBuff[ucRcvBufIndex].usLen);//malloc(sizeof(MAIN_CANITF_STRU) + CanRcvBuff[ucRcvBufIndex].len);
	if (NULL == pCanItfMsg)
	{
		return ;
	}
    ...
    ret = SndMsg(MSG_QUEUE_ID_MAIN,(SAT_MSG_HEAD *)pCanItfMsg);

    if (0 != ret)
    {
        SatFreeMsg(pCanItfMsg);
    }
}
...
static void CanItfRcvProc(void *lparam)
{
    ...
    int           ret = 0;
	tls_setkey(&pthread_tno_key,(int)pCanItf);
...
void CanItfProc(void* lpParam)
{
	CAN_ITF_STRU         *pCanItf = (CAN_ITF_STRU *)lpParam;
	int                  recvlen;
	...
	for(;;)
	{
		recvlen = msg_recv(MSG_QUEUE_ID_CANITF, (char**)&pMsgHead, NULL);
		if (recvlen < minSize) 
		{
			continue;
		}
		
		CanItfMsgProc(pCanItf,(char*)pMsgHead, pMsgHead->len, pMsgHead->event);
